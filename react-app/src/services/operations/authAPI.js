  ...
  return async (dispatch) => {
    ...
    try {
      const finalPayload = {
        ...
        confirmPassword: payload.password, // map same password for now
        account_type: payload.accountType, // backend expects this
        otp: payload.otp,
        image: "" // send empty string if you don’t upload image
      };

      console.log("FINAL SIGNUP PAYLOAD >>>", finalPayload);

      const response = await apiConnector("POST", SIGNUP_API, finalPayload);
